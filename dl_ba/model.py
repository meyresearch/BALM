...
from dl_ba.loss import BMCLoss
...
class BindingAffinityModel(BaseModel):
    ...
    def _set_loss(self):
        ...
        if self.loss_fn_type == "cosine_mse":
            return nn.MSELoss()
        if self.loss_fn_type == "cosine_balanced_mse":
            return BMCLoss(
                self.model_configs.model_hyperparameters.init_noise_sigma,
                device=self.protein_model.device,
            )
